...
class DetectLevel():
    def __init__(self):
        ...
        if self.is_calibration_mode == True:
            if self.pub_image_type == "compressed":
                ...
                self.pub_image_color_filtered = rospy.Publisher('/detect/image_output_sub1/compressed', CompressedImage,
                                                                queue_size=1)
            elif self.pub_image_type == "raw":
    ...
    def cbLevelCrossingOrder(self, order):
        ...
        #输入：/detect/level_crossing_order
        #根据输入的检测到的拦路杆的状态，发布max_vel对小车进行控制，返回目前的状态
    ...
    def fnFindLevel(self):
        #对图像进行掩膜并高斯滤波
        #is_level_detected, is_level_close, is_level_opened的布尔值
    ...
    def fnFindRectOfLevel(self, mask):
        #利用blob斑点检测算法，检测关键点
        #当检测到3个红色矩形框时，计算关键点的平均坐标以及关键点到平均点的距离列表
        #通过计算三个红色块之间的距离以及校验三个色块的线性情况判断是否检测到交通杆
        #计算交通杆的斜率，根据斜率值的大小说明到拦路杆的距离，判断拦路杆的状态
        #通过计算self.stop_bar_count的判断交通杆已经收起，小车可以通过
        #返回交通杆的三个状态（小车减速，停车，通过）
