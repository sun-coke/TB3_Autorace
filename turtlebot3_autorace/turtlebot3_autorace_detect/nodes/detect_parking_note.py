...
class DetectParking():
    def __init__(self):
        ...
        self.sub_parking_lot_order = rospy.Subscriber('/detect/parking_lot_order', UInt8, self.cbParkingLotOrder,
                                                      queue_size=1)

        self.sub_scan_obstacle = rospy.Subscriber('/scan', LaserScan, self.cbScanObstacle, queue_size=1)

        self.sub_parking_finished = rospy.Subscriber('/control/parking_finished', UInt8, self.cbParkingFinished,
                                                     queue_size=1)
        ...
        if self.sub_image_type == "compressed":
            ...
            self.sub_image_original = rospy.Subscriber('/camera/image_projected_compensated', Image, self.cbGetImage, queue_size=1)
        ...
        if self.pub_image_type == "compressed":
            ...
            self.pub_image_parking = rospy.Publisher('/detect/image_parking', Image, queue_size=1)
        ...
        self.StepOfParkingLot = Enum('StepOfParkingLot',
                                     'searching_parking_sign '
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__)) #返回目录名，返回绝对路径
    ...
    def cbParkingLotOrder(self, order):
        ...
        #根据不同在self.pub_parking_lot_return上发布泊车信息
        #发布max_vel指令
        ...
        if order.data == self.StepOfParkingLot.searching_parking_sign.value:
            ...
            msg_pub_parking_lot_return.data = self.StepOfParkingLot.searching_parking_point_line.value ##反馈core目前泊车动作
            ...
            #通过while循环保证小车速度，当检测到第一个停车位为空时，小车速度为零
    ...
    def fnFindDotLine(self):
        ...
        #利用停车位标志线是虚线段，采用blob斑点检测算法对地面标志线进行检测
        #检测到停车线，返回真值
...
if __name__ == '__main__':
    rospy.init_node('detect_parking_note')
    node = DetectParking()
    node.main()
