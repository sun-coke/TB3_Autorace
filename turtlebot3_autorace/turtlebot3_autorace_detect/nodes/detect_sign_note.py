...
class DetectSign():
    ...
    def fnPreproc(self):
        ...
        #初始化一个SIFT检测器
        #读取路标，利用SIFT提取图片的关键点和特征符
        #利用FlannBasedMatcher算法（二维特征点匹配）进行近似匹配得到self.flann值
        ...
        search_params = dict(checks = 50)   #用它来指定递归遍历的次数。值越高结果越准确，但是消耗的时间也越多

        self.flann = cv2.FlannBasedMatcher(index_params, search_params)
        #它是一种近似法，算法更快但是找到的是最近邻近似匹配
    ...
    def cbFindTrafficSign(self, image_msg):
        ...
        MIN_MATCH_COUNT = 9   #设定最小匹配点个数，达到匹配个数要求才能开始寻找目标（设定MIN_MATCH_COUNT参数）
        ...
        if len(good2)>MIN_MATCH_COUNT:
            ...
            #提取匹配的对应点的坐标信息,计算3x3映射矩阵(单应性矩阵H)
        ...
        if len(good3)>MIN_MATCH_COUNT:
            ...
            if mse < MIN_MSE_DECISION:
                ...
                cv2.imshow("window", matchesMask3)
                cv2.waitKey(3)
...
if __name__ == '__main__':
    rospy.init_node('detect_sign_note')
    node = DetectSign()
    node.main()
